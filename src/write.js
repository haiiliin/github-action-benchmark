...
function addBenchmarkToDataJson(benchName, bench, data, config) {
    ...
    if (data.entries[benchName] === undefined) {
        ...
        if (maxItemsInSuite !== null && suites.length > maxItemsInSuite) {
            ...
            core.debug(`Number of data items for '${benchName}' was truncated to ${maxItemsInSuite} due to max-data-items`);
        }
    }
    return prevBench;
}
...
async function writeBenchmarkToGitHubPages(bench, config) {
    ...
    const assets = {
        'index.html': await fs_1.promises.readFile(path.join(__dirname, 'assets/index.html'), 'utf8'),
        'benchmark.css': await fs_1.promises.readFile(path.join(__dirname, 'assets/benchmark.css'), 'utf8'),
        'main.js': await fs_1.promises.readFile(path.join(__dirname, 'assets/main.js'), 'utf8'),
        'funcs.js': await fs_1.promises.readFile(path.join(__dirname, 'assets/funcs.js'), 'utf8'),
    };
    if (config.configDataJsonPath) {
        assets['config.js'] = 'window.CONFIGURATION_DATA = ' + (await fs_1.promises.readFile(config.configDataJsonPath, 'utf8'));
    }
    else {
